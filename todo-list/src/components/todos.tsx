...
export default function Todos({ todos, setTodo}: { todos: ITodo[], setTodo: (todos: ITodo[]) => void}) {

  function handleDelete(idx: number) {
    setTodo([...todos.filter((_, id) => id !== idx)])
  }

  function handleCheck(e: any, idx: number) {
    const updateTodos = todos.map((item, id) =>
      id === idx ? { ...item, isDone: e.target.checked } : item
    )
        ...
        todos.map((item, idx) => {
          return (
            <div key={idx} className="flex justify-between mb-1">
              <input onChange={(e) => handleCheck(e, idx)} type="checkbox" className="hover:cursor-pointer" checked={item.isDone}/>
              <div>{item.desc}</div>
              <button
                className="hover:bg-slate-300 w-8 h-8 flex justify-center items-center rounded-md border-solid border-red-500 border-2"
